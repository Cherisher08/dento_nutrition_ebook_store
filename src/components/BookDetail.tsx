    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16 mt-6">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex justify-between w-full max-w-sm px-4">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs font-semibold text-gray-600">{book.details.pages || 'N/A'} Pages</span>
          </div>
          <div className="flex flex-col items-center">
        ...
        <div className="flex gap-4 mt-8 w-full max-w-sm">
          <button className="flex-1 py-3 px-6 rounded-xl cursor-pointer bg-cyan-200 font-bold text-gray-900 hover:bg-cyan-300 transition-colors shadow-sm">
            Buy ₹{book.price} {book.originalPrice && <span className='line-through pl-2 text-gray-600'>₹{book.originalPrice}</span>}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-center">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs font-bold rounded uppercase tracking-wide">{book.publisher}</span>
        </div>

        {book.rating && <div className="flex items-center gap-1 mb-8">
          {[...Array(5)].map((_, i) => (
            <svg key={i} className={`h-5 w-5 ${i < Math.floor(book.rating ?? 0) ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
